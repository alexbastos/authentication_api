...
* **Relacionamentos:**
  * `socialAccounts`: Lista de contas sociais (ex: Google, Apple) atreladas ao usuário.
  * `refreshTokens`: Histórico de tokens de sessão emitidos para o usuário.
...
| **POST** | `/auth/refresh` | Troca um Refresh Token válido por um novo par de tokens. Implementa **Token Rotation** (o token antigo é invalidado no banco imediatamente por segurança). |
