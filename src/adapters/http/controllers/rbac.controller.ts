...
import type { ListRolesUseCase } from '../../../application/use-cases/rbac/list-roles.use-case.js';
import type { GetRoleUseCase } from '../../../application/use-cases/rbac/get-role.use-case.js';
import type {
...
export class RbacController {
  constructor(
    ...
    private readonly getRoleUC: GetRoleUseCase,
  ) {}

  async listPermissions(request: FastifyRequest<{ Querystring: { category?: string } }>, reply: FastifyReply) {
    const result = await this.listPermissionsUC.execute(request.user!.role, request.query.category);
  ...
  async listRoles(request: FastifyRequest, reply: FastifyReply) {
    const result = await this.listRolesUC.execute(request.user!.role);
  ...
  async getRole(request: FastifyRequest<{ Params: RoleIdParams }>, reply: FastifyReply) {
    const result = await this.getRoleUC.execute(request.params.id, request.user!.role);
  ...
  async deleteRole(request: FastifyRequest<{ Params: RoleIdParams }>, reply: FastifyReply) {
    await this.deleteRoleUC.execute(request.params.id, request.user!.role);
  ...
  async removeRoleFromUser(request: FastifyRequest<{ Params: UserRoleParams }>, reply: FastifyReply) {
    await this.removeRoleUC.execute(request.params.id, request.params.roleId, request.user!.role);
    return reply.status(204).send();
  }

  async getUserPermissions(request: FastifyRequest<{ Params: UserIdParams }>, reply: FastifyReply) {
    const result = await this.getUserPermissionsUC.execute(request.params.id, request.user!.role);
